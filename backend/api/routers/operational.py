    ...
    if not pred_df.empty:
        ...
        if len(high_risk) >= 5:
            alerts.append({
                ...
                "zone_id":    None,
                "severity":   "high",
                "message":    f"{len(high_risk)} zones currently at high depletion risk",
                "created_at": now,
            })
        if drift_flag:
            alerts.append({
...
def get_zones(risk_level: Optional[str] = None, region: Optional[str] = None):
    ...
    for _, row in pred_df.iterrows():
        ...
        zones.append({
            ...
            "depletion_risk_score": float(_clean(row.get("depletion_risk_score", row.get("delay_risk_score")), 0.0)),
            "demand_pressure_score": float(_clean(row.get("demand_pressure_score"), 0.0)),
            "demand_pressure_level": str(_clean(row.get("demand_pressure_level"), "low")),
            "imbalance_score": float(_clean(row.get("imbalance_score"), 0.0)),
            "imbalance_level": str(_clean(row.get("imbalance_level"), "low")),
            "policy_action":       str(_clean(row.get("policy_action"), "no_action")),
            "policy_reason":       str(_clean(row.get("policy_reason"), "")),
            "predicted_shortage": float(_clean(row.get("predicted_shortage"), 0.0)),
            "severity_bucket": str(_clean(row.get("severity_bucket"), "low")),
            "persistence_count": int(_clean(row.get("persistence_count"), 0)),
